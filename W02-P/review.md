**새로배운 내용**
    ...
    - 웹서버는 기본적으로 html해석 기능은 있지만 php코드는 해석할 수 없다.
    - php코드에서 DB에 접근하기 위해서는 mysql에 요청해야한다.

    - 클라이언트와 서버, php, mysql 관계
        - 클라이언트가 네트워크를 통해 웹서버로 정보를 요청
        - 웹서버는 php코드를 처리할 수 없기 때문에 php프로그램에 처리를 요청한다. 
        - php에서 DB에 저장된 값을 사용하기위해서는 mysql에 쿼리를 보낸다.
...
**회고**
    ...
    (+) 윈도우 터미널을 왜 이제야 알았을까.. cmd창 불편했는데 이걸로 리눅스 명령어도 사용할 수 있어서 편리했고, 무엇보다 탭기능이 신박했다.
